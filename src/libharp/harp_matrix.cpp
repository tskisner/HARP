...
void harp::sub_block ( matrix_dist & in, int firstrow, int firstcol, int nrow, int ncol, matrix_dist & out ) {

  // FIXME: this should be changed to not store a full local copy of the output!

  matrix_local in_loc ( nrow, ncol );
  local_matrix_zero ( in_loc );

  elem::AxpyInterface < double > globloc;
  globloc.Attach( elem::GLOBAL_TO_LOCAL, in );
  globloc.Axpy ( 1.0, in_loc, firstrow, firstcol );
  globloc.Detach();

  out.ResizeTo ( nrow, ncol );

  // Update local output matrix

  matrix_local & out_loc = out.LocalMatrix();

  int hlocal = out.LocalHeight();
  int wlocal = out.LocalWidth();

  int rowoff = out.ColShift();
  int rowstride = out.ColStride();
  int row;

  int coloff = out.RowShift();
  int colstride = out.RowStride();
  int col;
  ...
  for ( int i = 0; i < wlocal; ++i ) {
    for ( int j = 0; j < hlocal; ++j ) {
      row = rowoff + j * rowstride;
      col = coloff + i * colstride;
      mval = in_loc.Get ( row, col );
      out_loc.Set ( j, i, mval );
    }
  }
