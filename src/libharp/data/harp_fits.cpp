...
void harp::fits::key_read ( fitsfile * fp, std::string const & keyname, float & val ) {
  ...
  ret = fits_read_key ( fp, TFLOAT, keycopy, (void*)&val, comment, &status );
  if ( status != 0 ) {
    val = 0.0;
  }
...
void harp::fits::key_write ( fitsfile * fp, std::string const & keyname, float const & keyval, std::string const & keycom ) {
