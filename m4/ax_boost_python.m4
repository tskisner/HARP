...
# modified 2011-06-06 by Ted Kisner <tskisner@lbl.gov> to actually use the boost
# include and linking flags during checks.  Also use variables from a better
# python autoconf macro.
...
if test "$ac_cv_boost_python" = "yes"; then
  ...
  LDFLAGS="$BOOST_LDFLAGS $PYTHON_LDFLAGS $LDFLAGS"
  for ax_lib in $ax_python_lib $ax_boost_python_lib boost_python; do
    AC_CHECK_LIB($ax_lib, exit, [BOOST_PYTHON_LIBS="-l$ax_lib"])
  done
